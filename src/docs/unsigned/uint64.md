...
| Minimum value    | 0                                              |
| Maximum value    | 2<sup>64</sup>−1 ≈ 1.844674 × 10<sup>19</sup>  |
| Literal suffix   | `u64`                                          |
| Representation   | Binary, wraps modulo 2<sup>64</sup>            |
| Hardware support | Native on all supported targets                |

`uint64` is a full-width unsigned integer matching the native word of 64-bit targets — a fixed-width type whose size is identical on every target platform. It stores non-negative values in plain binary and wraps modulo 2<sup>64</sup> on overflow.
...
If a literal value does not fit in `uint64`, the compiler emits an error at compile time. See [Literals](/docs/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
**Overflow.** Unsigned arithmetic wraps modulo 2<sup>64</sup> in **both** debug and release builds — it never triggers a fatal error. Subtracting past zero wraps to a large value rather than going negative (`0 - 1` yields 2<sup>64</sup>−1), so prefer a signed type for any value that can legitimately fall below zero.
...
- [`int64`](/docs/signed/int64) — signed counterpart of the same width
