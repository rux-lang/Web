...
| Minimum value    | 0                                               |
| Maximum value    | 2<sup>256</sup>−1 ≈ 1.157921 × 10<sup>77</sup>  |
| Literal suffix   | `u256`                                          |
| Representation   | Binary, wraps modulo 2<sup>256</sup>            |
| Hardware support | Software-emulated on all targets                |

`uint256` is a big-integer type for cryptographic values — a fixed-width type whose size is identical on every target platform. It stores non-negative values in plain binary and wraps modulo 2<sup>256</sup> on overflow. No mainstream CPU has native 256-bit arithmetic, so every operation is software-emulated; reach for it when correctness and portability matter more than raw throughput.
...
If a literal value does not fit in `uint256`, the compiler emits an error at compile time. See [Literals](/docs/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
**Overflow.** Unsigned arithmetic wraps modulo 2<sup>256</sup> in **both** debug and release builds — it never triggers a fatal error. Subtracting past zero wraps to a large value rather than going negative (`0 - 1` yields 2<sup>256</sup>−1), so prefer a signed type for any value that can legitimately fall below zero.
...
- [`int256`](/docs/signed/int256) — signed counterpart of the same width
