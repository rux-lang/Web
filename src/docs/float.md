...
> **Rux Language Reference** · Primitive Types · Float
...
```rux
let x: float32 = 3.14
let y: float64 = 2.718_281_828_459_045
...
| Type       | Width   | Significand Bits | Exponent Bits | Approx. Decimal Digits | Range (approx.) |
| ---------- | ------- | ---------------- | ------------- | ---------------------- | --------------- |
| `float8`   | 8-bit   | 3                | 4             | ~1–2                   | ±240.0          |
| `float16`  | 16-bit  | 10               | 5             | ~3–4                   | ±65,504         |
| `float32`  | 32-bit  | 23               | 8             | ~7                     | ±3.4 × 10³⁸     |
| `float64`  | 64-bit  | 52               | 11            | ~15–16                 | ±1.8 × 10³⁰⁸    |
| `float80`  | 80-bit  | 64               | 15            | ~18–19                 | ±1.2 × 10⁴⁹³²   |
| `float128` | 128-bit | 112              | 15            | ~33–34                 | ±1.2 × 10⁴⁹³²   |
| `float256` | 256-bit | 236              | 19            | ~71–72                 | ±~10⁷⁸⁹¹³       |
| `float512` | 512-bit | 492              | 19            | ~148–149               | ±~10⁷⁸⁹¹³       |
...
```rux
let pi: float32 = 3.141_592_7
```

### `float64`

Double-precision. The default float type in Rux (see [Default Float Type](#default-float-type)). Suitable for scientific computing, financial calculations, and most general-purpose work where precision matters.

```rux
let e: float64 = 2.718_281_828_459_045
```
...
```rux
let precise: float80 = 1.234_567_890_123_456_789
```

### `float128`

Quadruple-precision (IEEE 754 binary128). Useful for compensated summation, high-accuracy numerical integration, and as a reference implementation when validating lower-precision algorithms. Software-emulated on most hardware.

```rux
let quad: float128 = 1.0q   // 'q' suffix denotes float128 literal
```
...
```rux
let ultra: float256 = 1.0o   // 'o' suffix denotes float256 literal
```
...
let d = 1.0q          // float128 via suffix
```
...
let a = 1.0
let b = -0.5
let c = 3.141_592_653_589_793   // underscores allowed as separators
```
...
```rux
let a: float64 = 3.141_592_653_589_793
...
let j: int32   = f.round() as int32  // rounds to nearest  → 43
let k: int32   = f.floor() as int32  // floor              → 42
let l: int32   = f.ceil()  as int32  // ceiling            → 43
...
> Division by zero does **not** panic; it produces `Inf` or `NaN` per IEEE 754.
...
Always use `Float.is_nan()` to check for `NaN`:
...
```rux
let a: float64 = 1.000_000_001
let b: float64 = 1.000_000_000
...
```rux
let bytes = (3.14f32).to_le_bytes()  // [u8; 4]
