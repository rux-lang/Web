...
`Result` is the recoverable-error half of Rux's error model; for conditions a caller is not expected to recover from, see [`Panic`](/api/rux/panic).
...
- [`Panic`](/api/rux/panic) — for unrecoverable errors
