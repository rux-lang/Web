...
::warning
Do not use the return value as a length until [`IsError`](/api/linux/iserror) has ruled out
a negative result.
::
...
- [`Write`](/api/linux/write) — write bytes to a descriptor
- [`Close`](/api/linux/close) — close a descriptor
