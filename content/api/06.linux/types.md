...
[`Nanosleep`](/api/linux/nanosleep) reads it as a relative duration; [`ClockGetTime`](/api/linux/clockgettime)
writes a timestamp for the selected clock.

## See also

- [`Linux`](/api/linux) — the package overview
- [`Mmap`](/api/linux/mmap) — uses the protection and mapping flags
- [`ClockGetTime`](/api/linux/clockgettime) — uses the clock IDs and `Timespec`
- [`Syscall0`–`Syscall6`](/api/linux/syscalls) — use the syscall numbers directly
