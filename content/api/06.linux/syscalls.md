...
`int64` — the raw kernel result. Use [`IsError`](/api/linux/iserror) before interpreting
...
- [`Constants`](/api/linux/types) — exported syscall numbers
- [`IsError`](/api/linux/iserror) — test the raw result
- [`Errno`](/api/linux/errno) — decode an error result
