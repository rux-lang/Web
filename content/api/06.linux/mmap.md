...
`offset`. Release a successful mapping with [`Munmap`](/api/linux/munmap).

::caution
Call [`IsError`](/api/linux/iserror) before casting the result to a pointer. Casting a
...
- [`Constants`](/api/linux/types) — available protection and mapping flags
- [`Munmap`](/api/linux/munmap) — remove a mapping
