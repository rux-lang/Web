...
An arithmetic type capable of representing a calendar time, in seconds since the epoch. Used by [`time`](/api/c/time), [`ctime`](/api/c/ctime), [`gmtime`](/api/c/gmtime), [`localtime`](/api/c/localtime), [`mktime`](/api/c/mktime), and [`difftime`](/api/c/difftime).
...
An arithmetic type capable of representing processor time. Returned by [`clock`](/api/c/clock).
...
Calendar time broken into its components. On Windows the layout stops at `tm_isdst`; the `tm_gmtoff` and `tm_zone` fields exist only on the Unix targets. Filled by [`gmtime`](/api/c/gmtime) and [`localtime`](/api/c/localtime), and consumed by [`asctime`](/api/c/asctime), [`mktime`](/api/c/mktime), and [`strftime`](/api/c/strftime).
