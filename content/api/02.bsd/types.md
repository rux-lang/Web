...
constants with [`ClockGetTime`](/api/bsd/clockgettime).
...
Used by [`Nanosleep`](/api/bsd/nanosleep) and [`ClockGetTime`](/api/bsd/clockgettime).

## See also

- [`Bsd`](/api/bsd) — the package overview
- [`Mmap`](/api/bsd/mmap) — uses the protection and mapping flags
- [`ClockGetTime`](/api/bsd/clockgettime) — uses the clock IDs and `Timespec`
- [`Syscall0`–`Syscall6`](/api/bsd/syscalls) — use the syscall numbers directly
