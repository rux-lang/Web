...
or resource management is performed. Use [`IsError`](/api/bsd/iserror) and
[`Errno`](/api/bsd/errno) to interpret it.
...
| ----------------------------- | ------------------------------------------------------ |
| [`Errno`](/api/bsd/errno)     | Return the positive errno of a failing result, else 0. |
| [`IsError`](/api/bsd/iserror) | Test whether a result is a negative errno.             |

## See also

- [`Bsd`](/api/bsd) — the package overview
- [`Types and constants`](/api/bsd/types) - shared syscall numbers
