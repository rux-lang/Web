...
The wrappers return the kernel result directly: a non-negative value on success — a byte count, a process ID, a mapped address, or `0` — and a **negative errno** (`-1` through `-4095`) on failure. [`IsError`](/api/bsd/iserror) tests for that negative range and [`Errno`](/api/bsd/errno) turns it back into a positive errno number, so a legitimate positive result is never mistaken for an error.
...
| ------------------------- | ---------------------------------- |
| [`Read`](/api/bsd/read)   | Read bytes from a file descriptor. |
| [`Write`](/api/bsd/write) | Write bytes to a file descriptor.  |
| [`Close`](/api/bsd/close) | Close a file descriptor.           |

### Memory

| Function                    | Description                       |
| --------------------------- | --------------------------------- |
| [`Mmap`](/api/bsd/mmap)     | Create a virtual-memory mapping.  |
| [`Munmap`](/api/bsd/munmap) | Remove a virtual-memory mapping.  |
| [`Brk`](/api/bsd/brk)       | Change the process program break. |

### Process

| Function                    | Description                        |
| --------------------------- | ---------------------------------- |
| [`Exit`](/api/bsd/exit)     | Terminate the process immediately. |
| [`GetPid`](/api/bsd/getpid) | Return the calling process ID.     |

### Time

| Function                                | Description                      |
| --------------------------------------- | -------------------------------- |
| [`ClockGetTime`](/api/bsd/clockgettime) | Read a BSD clock.                |
| [`Nanosleep`](/api/bsd/nanosleep)       | Suspend for a relative interval. |

### Raw syscalls

| Function                                   | Description                                |
| ------------------------------------------ | ------------------------------------------ |
| [`Syscall0`–`Syscall6`](/api/bsd/syscalls) | Invoke an arbitrary syscall by number.     |
| [`IsError`](/api/bsd/iserror)              | Test whether a result is a negative errno. |
| [`Errno`](/api/bsd/errno)                  | Extract the positive errno from a result.  |

## Types and constants

The standard descriptors, syscall numbers, mapping and protection flags, clock IDs, and the [`Timespec`](/api/bsd/types) structure are listed on the [types and constants](/api/bsd/types) page.
