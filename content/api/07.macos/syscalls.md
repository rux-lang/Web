...
or resource management is performed. Use [`IsError`](/api/macos/iserror) and
[`Errno`](/api/macos/errno) to interpret it.
...
| ------------------------------- | ------------------------------------------------------ |
| [`Errno`](/api/macos/errno)     | Return the positive errno of a failing result, else 0. |
| [`IsError`](/api/macos/iserror) | Test whether a result is a negative errno.             |

## See also

- [`MacOS`](/api/macos) — the package overview
- [`Types and constants`](/api/macos/types) - class-qualified syscall numbers
