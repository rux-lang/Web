...
::caution
Check the result with [`IsError`](/api/macos/iserror) before casting it to a pointer.
Release every successful mapping with [`Munmap`](/api/macos/munmap) using its correct base
address and length.
::
...
- [`Munmap`](/api/macos/munmap) - remove a mapping
- [`Types and constants`](/api/macos/types) - protection and mapping flags
