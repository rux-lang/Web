...
The comparison is byte for byte and case-sensitive, on the same terms as [`Equals`](/api/text/string/equals).
...
- [`EndsWith`](/api/text/string/endswith) — the same test at the other end
- [`IndexOf`](/api/text/string/indexof) — find the bytes anywhere, not only at the front
- [`Equals`](/api/text/string/equals) — compare the whole string
