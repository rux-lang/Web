...
This is [`IndexOf`](/api/text/string/indexof) with the offset thrown away, and it costs the same — ask [`IndexOf`](/api/text/string/indexof) instead when the position matters.
...
- [`IndexOf`](/api/text/string/indexof) — the same search, reporting where the match is
