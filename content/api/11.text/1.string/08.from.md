...
The result has to be released with [`Free`](/api/text/string/free).
...
- [`New`](/api/text/string/new) — the empty string, without a copy
- [`Clone`](/api/text/string/clone) — copy a `String` rather than a buffer
- [`Free`](/api/text/string/free) — release the block when the string is finished with
