...
The block is sized up front — it goes through a [`StringBuilder`](/api/text/stringbuilder) built with [`WithCapacity`](/api/text/stringbuilder/withcapacity) — so this costs **one** allocation rather than one per round.
...
- [`+`](/api/text/string/plus) — join two different strings
- [`StringBuilder::WithCapacity`](/api/text/stringbuilder/withcapacity) — the one allocation this leans on
