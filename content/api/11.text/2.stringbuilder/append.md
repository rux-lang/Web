...
The bytes are **copied**, so an appended [`String`](/api/text/string) is left untouched and is still the caller's to free.
...
Growing may move the block, so a pointer taken from [`Data`](/api/text/stringbuilder/data) before an append must not be used after one.
...
- [`Reserve`](/api/text/stringbuilder/reserve) — make room before a run of appends
- [`IntoString`](/api/text/stringbuilder/intostring) — take the result once the appends are done
- [`String::+`](/api/text/string/plus) — join two strings, at the cost of an allocation each time
