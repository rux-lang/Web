...
`true` for space (`' '`), tab (`'\t'`), newline (`'\n'`), and carriage return (`'\r'`), and `false` for every other byte. These four are exactly the bytes [`String::Trim`](/api/text/string/trim) removes.
...
- [`String::Trim`](/api/text/string/trim) — drop the whitespace at both ends of a string
