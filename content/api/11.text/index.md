...
The package provides strings and fundamental text manipulation — an immutable [`String`](/api/text/string), and a [`StringBuilder`](/api/text/stringbuilder) to accumulate one.
...
There are no destructors, so a block is released by hand. Every `String` and `StringBuilder` this package hands out owns its block and has to be passed to `Free` exactly once — [`String::Free`](/api/text/string/free) or [`StringBuilder::Free`](/api/text/stringbuilder/free), not [`Memory::Free`](/api/memory/free).
...
A `String` never changes once built: every transformation returns a fresh one and leaves the receiver alone. That is what makes the value semantics safe, since a `String` built from a literal would otherwise be written through a read-only page. It also makes `+` in a loop a chain of allocations — build with a [`StringBuilder`](/api/text/stringbuilder) instead, and hand the result over with [`IntoString`](/api/text/stringbuilder/intostring) to avoid a final copy.

## Text encoding

A `String` holds bytes, and [`Length`](/api/text/string/length) counts bytes rather than characters, so a multi-byte UTF-8 sequence counts for more than one. [`At`](/api/text/string/at) and [`Substring`](/api/text/string/substring) index by byte and will happily cut a sequence in half. [`ToUpper`](/api/text/string/toupper) and [`ToLower`](/api/text/string/tolower) fold ASCII only and copy everything else through untouched.

The block carries no null terminator. Pair [`Data`](/api/text/string/data) with [`Length`](/api/text/string/length) when handing the bytes to something that reads them directly, rather than letting it scan for a terminator that is not there.

## Types

| Type                                       | Description                                          |
| ------------------------------------------ | ---------------------------------------------------- |
| [`String`](/api/text/string)               | An immutable string, owning the block behind it.     |
| [`StringBuilder`](/api/text/stringbuilder) | A growable buffer, for building a `String` in steps. |

## Functions

| Function                       | Description                                    |
| ------------------------------ | ---------------------------------------------- |
| [`IsSpace`](/api/text/isspace) | Whether a byte is one of the whitespace bytes. |
