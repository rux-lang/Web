...
This is exactly the negation of [`IsNan`](/api/format/isnan) or [`IsInfinite`](/api/format/isinfinite) — the three partition the values, and every value answers `true` to precisely one of them. A NaN is not finite: the test is a pair of comparisons against the range of the type, and every comparison against a NaN is false.

As with [`IsInfinite`](/api/format/isinfinite), the width matters, and the overload is chosen by the argument. A `float32` is measured against the `float32` range.
...
- [`IsNan`](/api/format/isnan) — whether the value is a NaN
- [`IsInfinite`](/api/format/isinfinite) — whether the value is one of the two infinities
- [`ToString`](/api/format/tostring) — what each of the three classes prints as
