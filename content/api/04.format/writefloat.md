...
Writes the same text [`ToString`](/api/format/tostring) would produce, into the builder rather than into a `String` of its own, so a caller who is already accumulating pays no allocation for it.
...
- [`ToString`](/api/format/tostring) — the same text in a `String` of its own
- [`IsNan`](/api/format/isnan) / [`IsInfinite`](/api/format/isinfinite) — classify a value before writing it
