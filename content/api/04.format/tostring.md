...
Each type is also extended with [`Stringable`](/api/format/stringable), so the same conversion is reachable as a method: `42.ToString()` and `ToString(42)` are the same call.
...
- [`Stringable`](/api/format/stringable) — put a `ToString` on a type of your own
- [`WriteInt`](/api/format/writeint) / [`WriteFloat`](/api/format/writefloat) — the same text, appended to a builder, with no `String` allocated
- [`IsNan`](/api/format/isnan) — ask about a value rather than reading `NaN` back out of its text
