...
`Error` holds a [`ParseError`](/api/format/parseerror) saying why the text was rejected.
...
- [`TryParseInt64`](/api/format/tryparseint64) — the same parse, into an out-parameter, without a `Result`
- [`ParseFloat64`](/api/format/parsefloat64) — parse a floating-point value instead
- [`ParseError`](/api/format/parseerror) — the reason a parse failed
- [`ToString`](/api/format/tostring) — the inverse, a value to its text
