...
Nothing is allocated here beyond whatever growth the builder needs, which is the point: [`ToString`](/api/format/tostring) allocates a `String` per value, and a caller who is already accumulating would only free it again. The digits go straight into the block the builder already has.
...
- [`WriteUint`](/api/format/writeuint) — the same, without a sign to write
