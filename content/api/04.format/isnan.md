...
Because every comparison against a NaN is false, a NaN is not infinite and not finite either — [`IsInfinite`](/api/format/isinfinite) and [`IsFinite`](/api/format/isfinite) both answer `false` for one, so the three functions partition the values rather than overlapping.
...
- [`IsFinite`](/api/format/isfinite) — whether the value is an ordinary number
- [`ToString`](/api/format/tostring) — a NaN converts to the text `NaN`
