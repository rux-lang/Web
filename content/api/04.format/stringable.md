...
Building the text with a [`StringBuilder`](/api/text/stringbuilder) and the [`Write`](/api/format/writeint) functions is what keeps the conversion to one allocation. Reaching for [`ToString`](/api/format/tostring) on each field instead would allocate a `String` per field and throw each one away again.
...
- [`ToString`](/api/format/tostring) — the conversion the primitives already have
- [`WriteInt`](/api/format/writeint) — append into a builder, without a `String` of its own
