...
This is the same parse as [`ParseInt64`](/api/format/parseint64) — the same accepted form
...
[`ParseError`](/api/format/parseerror) a failure was.
...
- [`ParseInt64`](/api/format/parseint64) — the same parse, returning a `Result` with the reason for a failure
- [`TryParseFloat64`](/api/format/tryparsefloat64) — the floating-point counterpart
