...
As with [`WriteInt`](/api/format/writeint), there is one overload rather than one per width: a narrower unsigned value widens to `uint64` on the way in, which loses nothing.
...
Nothing is allocated beyond whatever growth the builder needs. This is the function to reach for while accumulating; [`ToString`](/api/format/tostring) is the one that hands back a `String` of its own.
...
- [`WriteInt`](/api/format/writeint) — the same, with a sign for a negative value
- [`ToString`](/api/format/tostring) — the digits in a `String` of their own
