...
[`Set(ptr, size, 0)`](/api/memory/set), which is exactly how it is implemented everywhere
except Windows, where it lowers to `RtlZeroMemory`.

This is the usual companion to [`Alloc`](/api/memory/alloc), whose blocks come back
...
- [`Set`](/api/memory/set) — fill with a byte other than zero
- [`Alloc`](/api/memory/alloc) — the source of the uninitialized block
