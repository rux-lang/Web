...
[`Zero`](/api/memory/zero) first.

The block belongs to the caller and must be released exactly once with
[`Free`](/api/memory/free). Every successful call returns a distinct block, and the pointer
stays valid until it is passed to [`Free`](/api/memory/free) or moved by
[`Realloc`](/api/memory/realloc).
...
- [`Free`](/api/memory/free) — release the block when it is no longer needed
- [`Realloc`](/api/memory/realloc) — resize a block that already exists
- [`Zero`](/api/memory/zero) — clear the uninitialized bytes
