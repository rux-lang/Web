...
| ------ | --------- | ------------------------------------------------------------------ |
| `ptr`  | `*opaque` | A block from [`Alloc`](/api/memory/alloc) or `Realloc`, or `null`. |
...
- `Realloc(null, size)` allocates a fresh block, exactly like [`Alloc`](/api/memory/alloc).
...
- [`Copy`](/api/memory/copy) — move bytes between blocks by hand
