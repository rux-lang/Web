...
scratch block from [`Alloc`](/api/memory/alloc).
...
- [`Compare`](/api/memory/compare) — check that two blocks hold the same bytes
- [`Set`](/api/memory/set) — fill a block with a byte instead of copying one
- [`Realloc`](/api/memory/realloc) — resize a block, which copies the bytes for you
