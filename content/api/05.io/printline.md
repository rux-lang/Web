...
`PrintLine` mirrors every [`Print`](/api/io/print) overload and adds a form that takes nothing and writes only the newline:
...
Each overload is its [`Print`](/api/io/print) counterpart followed by a newline, and everything that page says about the text of a value, about `{}` substitution, and about a `String` argument staying yours to [`Free`](/api/text/string/free) holds here unchanged.
...
- [`Print`](/api/io/print) — the same overloads, with no newline after the value
