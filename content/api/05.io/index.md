...
The package provides standard input and output — a [`Print`](/api/io/print) and a [`PrintLine`](/api/io/printline) for every primitive type and for text, and a [`ReadLine`](/api/io/readline) that reads a line back.
...
[`ReadLine`](/api/io/readline) is the other way around. The `String` it returns is a fresh allocation the caller owns and passes to [`String::Free`](/api/text/string/free) exactly once, on the terms [`Text`](/api/text#ownership) sets out — including the line that came back empty.
...
| -------------------------------- | --------------------------------- |
| [`Print`](/api/io/print)         | Write a value to standard output. |
| [`PrintLine`](/api/io/printline) | The same, followed by a newline.  |

### Input

| Function                       | Description                                                  |
| ------------------------------ | ------------------------------------------------------------ |
| [`ReadLine`](/api/io/readline) | Read standard input through the next newline, as a `String`. |
