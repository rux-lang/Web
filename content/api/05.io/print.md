...
Nothing is appended — the next `Print` continues on the same line. Use [`PrintLine`](/api/io/printline) to end one.
...
The format overload substitutes each `{}` with the next argument, and writes a literal brace for `{{` and `}}`. The substitution is [`Format`](/api/format)'s, and the arguments are [`Stringable`](/api/format/stringable), so a type of your own can go into a `{}` as soon as it implements that interface.
...
- [`PrintLine`](/api/io/printline) — the same overloads, with a newline after the value
- [`ReadLine`](/api/io/readline) — read a line back from standard input
