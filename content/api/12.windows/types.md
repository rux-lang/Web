...
Pass one of these to [`GetStdHandle`](/api/windows/getstdhandle) to obtain a handle to the
...
Filled by [`GetLocalTime`](/api/windows/getlocaltime) (local time) and [`GetSystemTime`](/api/windows/getsystemtime) (UTC).
...
`fileSizeHigh` and `fileSizeLow` form the high and low halves of the unsigned 64-bit file size. The filename arrays hold null-terminated ANSI strings, and applications must not depend on the reserved fields. Populated by [`FindFirstFileA`](/api/windows/findfirstfilea) and [`FindNextFileA`](/api/windows/findnextfilea).

## See also

- [`Windows`](/api/windows) — the package overview
- [`CodePage`](/api/windows/codepage) — text-encoding identifiers for the conversion functions
