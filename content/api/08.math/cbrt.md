...
The real cube root of `x`. Unlike [`Sqrt`](/api/math/sqrt), the result is defined for a
...
- [`Pow`](/api/math/pow) — general exponentiation (`Pow(x, 1.0 / 3.0)` is not equivalent for negative `x`)
