...
- `Pow(x, 2.0)` and `Pow(x, 0.5)` (for non-negative `x`) are recognized and
  computed directly, as `x * x` and [`Sqrt(x)`](/api/math/sqrt).
...
- [`Sqrt`](/api/math/sqrt) — the common square-root case
- [`Cbrt`](/api/math/cbrt) — real cube roots, including negative arguments
