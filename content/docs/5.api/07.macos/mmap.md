...
::caution
Check the result with [`IsError`](/docs/api/macos/iserror) before casting it to a pointer.
Release every successful mapping with [`Munmap`](/docs/api/macos/munmap) using its correct base
address and length.
::
...
- [`MacOS`](/docs/api/macos) — the package overview
- [`Munmap`](/docs/api/macos/munmap) - remove a mapping
- [`Types and constants`](/docs/api/macos/types) - protection and mapping flags
