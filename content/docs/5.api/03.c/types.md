...
An arithmetic type capable of representing a calendar time, in seconds since the epoch. Used by [`time`](/docs/api/c/time), [`ctime`](/docs/api/c/ctime), [`gmtime`](/docs/api/c/gmtime), [`localtime`](/docs/api/c/localtime), [`mktime`](/docs/api/c/mktime), and [`difftime`](/docs/api/c/difftime).
...
An arithmetic type capable of representing processor time. Returned by [`clock`](/docs/api/c/clock).
...
Calendar time broken into its components. On Windows the layout stops at `tm_isdst`; the `tm_gmtoff` and `tm_zone` fields exist only on the Unix targets. Filled by [`gmtime`](/docs/api/c/gmtime) and [`localtime`](/docs/api/c/localtime), and consumed by [`asctime`](/docs/api/c/asctime), [`mktime`](/docs/api/c/mktime), and [`strftime`](/docs/api/c/strftime).
...
- [`C`](/docs/api/c) — the package overview
