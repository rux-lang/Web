...
The syscall ABI is platform- and architecture-specific. Prefer the cross-platform packages — [`Io`](/docs/api/io), [`Memory`](/docs/api/memory) — when they provide the operation you need.
...
The wrappers return the kernel result directly: a non-negative value on success — a byte count, a process ID, a mapped address, or `0` — and a **negative errno** (`-1` through `-4095`) on failure. [`IsError`](/docs/api/bsd/iserror) tests for that negative range and [`Errno`](/docs/api/bsd/errno) turns it back into a positive errno number, so a legitimate positive result is never mistaken for an error.
...
| ------------------------------ | ---------------------------------- |
| [`Read`](/docs/api/bsd/read)   | Read bytes from a file descriptor. |
| [`Write`](/docs/api/bsd/write) | Write bytes to a file descriptor.  |
| [`Close`](/docs/api/bsd/close) | Close a file descriptor.           |

### Memory

| Function                         | Description                       |
| -------------------------------- | --------------------------------- |
| [`Mmap`](/docs/api/bsd/mmap)     | Create a virtual-memory mapping.  |
| [`Munmap`](/docs/api/bsd/munmap) | Remove a virtual-memory mapping.  |
| [`Brk`](/docs/api/bsd/brk)       | Change the process program break. |

### Process

| Function                         | Description                        |
| -------------------------------- | ---------------------------------- |
| [`Exit`](/docs/api/bsd/exit)     | Terminate the process immediately. |
| [`GetPid`](/docs/api/bsd/getpid) | Return the calling process ID.     |

### Time

| Function                                     | Description                      |
| -------------------------------------------- | -------------------------------- |
| [`ClockGetTime`](/docs/api/bsd/clockgettime) | Read a BSD clock.                |
| [`Nanosleep`](/docs/api/bsd/nanosleep)       | Suspend for a relative interval. |

### Raw syscalls

| Function                                        | Description                                |
| ----------------------------------------------- | ------------------------------------------ |
| [`Syscall0`–`Syscall6`](/docs/api/bsd/syscalls) | Invoke an arbitrary syscall by number.     |
| [`IsError`](/docs/api/bsd/iserror)              | Test whether a result is a negative errno. |
| [`Errno`](/docs/api/bsd/errno)                  | Extract the positive errno from a result.  |

## Types and constants

The standard descriptors, syscall numbers, mapping and protection flags, clock IDs, and the [`Timespec`](/docs/api/bsd/types) structure are listed on the [types and constants](/docs/api/bsd/types) page.
