...
The size is a **total**, not an increment — it counts the bytes already written. [`Reserve`](/docs/api/text/stringbuilder/reserve) is the same thing stated relative to [`Length`](/docs/api/text/stringbuilder/length), and is usually what you want.

A `required` that already fits is left alone: the block never shrinks here. Otherwise the capacity doubles from a 16 byte floor until it covers `required`, which is why it can end up larger than what was asked for, and why a run of [`Append`](/docs/api/text/stringbuilder/append) calls costs an amortized constant rather than a reallocation each time.

Growing may move the block, so a pointer taken from [`Data`](/docs/api/text/stringbuilder/data) beforehand must not be used after.
