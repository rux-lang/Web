...
Whitespace is space, tab, newline, and carriage return, which is exactly what [`IsSpace`](/docs/api/text/isspace) reports.
...
- [`IsSpace`](/docs/api/text/isspace) — the bytes this treats as whitespace
- [`Substring`](/docs/api/text/string/substring) — drop bytes from the ends by offset instead
