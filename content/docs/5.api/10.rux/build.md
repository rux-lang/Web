...
`#build` is a compile-time value describing the profile and output of the current build. Read its fields in a [`when`](/docs/lang/comptime/conditional) to compile differently by mode, or embed the build date and time in a program.
...
| `debugAssertions` | `bool`             | Whether [`DebugAssert`](/docs/api/rux/assert) checks are compiled. |
...
- [`#target`](/docs/api/rux/target) / [`#compiler`](/docs/api/rux/compiler) / [`#config`](/docs/api/rux/config) / [`#source`](/docs/api/rux/source) — the other compile-time values
