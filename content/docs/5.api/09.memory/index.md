...
The block a caller receives is raw and owned: nothing tracks it, nothing frees it, and its contents are whatever the platform last left there. [`Alloc`](/docs/api/memory/alloc) does not zero it — use [`Zero`](/docs/api/memory/zero) when you need a clean block. Every block returned by [`Alloc`](/docs/api/memory/alloc) or [`Realloc`](/docs/api/memory/realloc) must be released exactly once with [`Free`](/docs/api/memory/free), and a pointer that [`Realloc`](/docs/api/memory/realloc) has moved must not be used again.
...
| ------------------------------------- | ---------------------------------------------- |
| [`Alloc`](/docs/api/memory/alloc)     | Allocate an uninitialized block of memory.     |
| [`Realloc`](/docs/api/memory/realloc) | Resize a block, preserving the bytes that fit. |
| [`Free`](/docs/api/memory/free)       | Release a block back to the platform.          |

### Filling

| Function                        | Description                        |
| ------------------------------- | ---------------------------------- |
| [`Set`](/docs/api/memory/set)   | Fill a block with a repeated byte. |
| [`Zero`](/docs/api/memory/zero) | Fill a block with zero bytes.      |

### Bulk operations

| Function                              | Description                                       |
| ------------------------------------- | ------------------------------------------------- |
| [`Copy`](/docs/api/memory/copy)       | Copy bytes between two non-overlapping blocks.    |
| [`Compare`](/docs/api/memory/compare) | Find the offset at which two blocks first differ. |
