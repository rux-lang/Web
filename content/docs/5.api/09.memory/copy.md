...
scratch block from [`Alloc`](/docs/api/memory/alloc).
...
- [`Compare`](/docs/api/memory/compare) — check that two blocks hold the same bytes
- [`Set`](/docs/api/memory/set) — fill a block with a byte instead of copying one
- [`Realloc`](/docs/api/memory/realloc) — resize a block, which copies the bytes for you
