...
| ------ | --------- | ----------------------------------------------------------------------- |
| `ptr`  | `*opaque` | A block from [`Alloc`](/docs/api/memory/alloc) or `Realloc`, or `null`. |
...
- `Realloc(null, size)` allocates a fresh block, exactly like [`Alloc`](/docs/api/memory/alloc).
- `Realloc(ptr, 0)` releases the block and returns `null`, exactly like
  [`Free`](/docs/api/memory/free).
...
- [`Alloc`](/docs/api/memory/alloc) — allocate the block in the first place
- [`Free`](/docs/api/memory/free) — release the block when it is no longer needed
- [`Copy`](/docs/api/memory/copy) — move bytes between blocks by hand
