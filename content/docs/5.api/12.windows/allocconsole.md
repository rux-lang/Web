...
[`GetLastError`](/docs/api/windows/getlasterror) after failure.

## Description

A process can be associated with at most one console. After allocation, obtain
the console handles with [`GetStdHandle`](/docs/api/windows/getstdhandle).

## See also

- [`Windows`](/docs/api/windows) — the package overview
- [`GetStdHandle`](/docs/api/windows/getstdhandle) — retrieve a standard device handle
