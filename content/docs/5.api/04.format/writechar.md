...
There is no `char8` overload, and none is needed: a `char8` is already one byte of UTF-8, so [`StringBuilder::Append`](/docs/api/text/stringbuilder/append) takes it as it stands, with nothing to encode.
...
- [`ToString`](/docs/api/format/tostring) — the character in a `String` of its own, and what a `char8` does instead
- [`Text::StringBuilder::Append`](/docs/api/text/stringbuilder/append) — append bytes that are already UTF-8
- [`Text::String::Length`](/docs/api/text/string/length) — bytes, not characters
