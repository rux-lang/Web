...
This is exactly the negation of [`IsNan`](/docs/api/format/isnan) or [`IsInfinite`](/docs/api/format/isinfinite) — the three partition the values, and every value answers `true` to precisely one of them. A NaN is not finite: the test is a pair of comparisons against the range of the type, and every comparison against a NaN is false.

As with [`IsInfinite`](/docs/api/format/isinfinite), the width matters, and the overload is chosen by the argument. A `float32` is measured against the `float32` range.
...
- [`IsNan`](/docs/api/format/isnan) — whether the value is a NaN
- [`IsInfinite`](/docs/api/format/isinfinite) — whether the value is one of the two infinities
- [`ToString`](/docs/api/format/tostring) — what each of the three classes prints as
