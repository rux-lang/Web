...
Each type is also extended with [`Stringable`](/docs/api/format/stringable), so the same conversion is reachable as a method: `42.ToString()` and `ToString(42)` are the same call.
...
A `String` holding the text of the value, in a block the caller owns and passes to [`String::Free`](/docs/api/text/string/free) exactly once.
...
- [`Stringable`](/docs/api/format/stringable) — put a `ToString` on a type of your own
- [`WriteInt`](/docs/api/format/writeint) / [`WriteFloat`](/docs/api/format/writefloat) — the same text, appended to a builder, with no `String` allocated
- [`IsNan`](/docs/api/format/isnan) — ask about a value rather than reading `NaN` back out of its text
