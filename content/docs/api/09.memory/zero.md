...
[`Set(ptr, size, 0)`](/docs/api/memory/set), which is exactly how it is implemented everywhere
except Windows, where it lowers to `RtlZeroMemory`.

This is the usual companion to [`Alloc`](/docs/api/memory/alloc), whose blocks come back
...
- [`Set`](/docs/api/memory/set) — fill with a byte other than zero
- [`Alloc`](/docs/api/memory/alloc) — the source of the uninitialized block
