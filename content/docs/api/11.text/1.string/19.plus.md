...
The result owns its block and has to be released with [`Free`](/docs/api/text/string/free) — including the intermediate results of a chain, which is what makes `a + b + c` three allocations and two leaks if only the last one is freed. In a loop it is a chain of allocations: accumulate with a [`StringBuilder`](/docs/api/text/stringbuilder) instead.
...
- [`StringBuilder::Append`](/docs/api/text/stringbuilder/append) — join without allocating on every step
- [`Repeat`](/docs/api/text/string/repeat) — join a string to itself N times, in one allocation
- [`Free`](/docs/api/text/string/free) — release the block the result owns
