...
- `Pow(x, 2.0)` and `Pow(x, 0.5)` (for non-negative `x`) are recognized and
  computed directly, as `x * x` and [`Sqrt(x)`](/docs/api/math/sqrt).
...
- [`Sqrt`](/docs/api/math/sqrt) — the common square-root case
- [`Cbrt`](/docs/api/math/cbrt) — real cube roots, including negative arguments
