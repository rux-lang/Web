...
Because every comparison against a NaN is false, a NaN is not infinite and not finite either — [`IsInfinite`](/docs/api/format/isinfinite) and [`IsFinite`](/docs/api/format/isfinite) both answer `false` for one, so the three functions partition the values rather than overlapping.
...
- [`IsFinite`](/docs/api/format/isfinite) — whether the value is an ordinary number
- [`ToString`](/docs/api/format/tostring) — a NaN converts to the text `NaN`
