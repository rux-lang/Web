...
This is the same parse as [`ParseInt64`](/docs/api/format/parseint64) — the same accepted form
...
[`ParseError`](/docs/api/format/parseerror) a failure was.
...
- [`ParseInt64`](/docs/api/format/parseint64) — the same parse, returning a `Result` with the reason for a failure
- [`TryParseFloat64`](/docs/api/format/tryparsefloat64) — the floating-point counterpart
