...
Writes the same text [`ToString`](/docs/api/format/tostring) would produce, into the builder rather than into a `String` of its own, so a caller who is already accumulating pays no allocation for it.
...
- [`Format`](/docs/api/format) — the package overview, and what the digit cutoff costs
- [`ToString`](/docs/api/format/tostring) — the same text in a `String` of its own
- [`IsNan`](/docs/api/format/isnan) / [`IsInfinite`](/docs/api/format/isinfinite) — classify a value before writing it
