...
[`Nanosleep`](/docs/api/linux/nanosleep) reads it as a relative duration; [`ClockGetTime`](/docs/api/linux/clockgettime)
writes a timestamp for the selected clock.

## See also

- [`Linux`](/docs/api/linux) — the package overview
- [`Mmap`](/docs/api/linux/mmap) — uses the protection and mapping flags
- [`ClockGetTime`](/docs/api/linux/clockgettime) — uses the clock IDs and `Timespec`
- [`Syscall0`–`Syscall6`](/docs/api/linux/syscalls) — use the syscall numbers directly
