...
`offset`. Release a successful mapping with [`Munmap`](/docs/api/linux/munmap).

::caution
Call [`IsError`](/docs/api/linux/iserror) before casting the result to a pointer. Casting a
...
- [`Constants`](/docs/api/linux/types) — available protection and mapping flags
- [`Munmap`](/docs/api/linux/munmap) — remove a mapping
