...
You can trigger one explicitly with [`Panic`](/api/rux/panic) from the `Rux` package, which prints a diagnostic message and the call-site location to standard error and exits with code `1`:
...
- [`Panic`](/api/rux/panic) — the standard-library function that triggers a fatal error
