...
When a `#source` field is the default value of a parameter, it expands at the **call site**, so a function learns where it was called from without the caller passing anything — the pattern behind [`Assert`](/api/rux/assert) and [`Panic`](/api/rux/panic). See [Source Location as a Default Argument](/docs/comptime/context#source-location-as-a-default-argument) for the full example.
