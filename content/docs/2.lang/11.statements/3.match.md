...
A pattern can be a literal, a range, or a destructuring of an [`enum`](/docs/lang/enums/overview) variant or a [`struct`](/docs/lang/structs/overview):
...
- [`if` / `else`](/docs/lang/statements/if) — simpler branching for one or two cases
- [Type Tests](/docs/lang/operations/type-test) — `is` for a single type check without exhaustiveness
- [Enumerations](/docs/lang/enums/overview) — the tagged values `match` most often destructures
