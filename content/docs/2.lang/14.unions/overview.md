...
Unions are **untagged**: the language does not track which field was written last, and reading a different field is not an error. The programmer is responsible for knowing which field is active. Prefer [`enum`](/docs/lang/enums/overview) for type-safe tagged unions — enum variants with data cover most cases where a union looks tempting.
...
- Interop with C `union` types through the [foreign function interface](/docs/lang/ffi/overview)
