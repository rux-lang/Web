...
`float64` is the default floating-point type in Rux — [`float`](/docs/lang/floating/float) is a built-in alias for it. It is fully [IEEE 754](https://en.wikipedia.org/wiki/IEEE_754) compliant. Special values (`Inf`, `-Inf`, `NaN`) are supported, and division by zero produces `Inf` or `NaN` rather than a fatal error.
...
See [Literals](/docs/lang/lexical/literals) for the full grammar.
...
`NaN` is unordered: every comparison involving it returns `false`, including `NaN == NaN`. Detect it with [`IsNan`](/docs/api/format/isnan), never `==`. For the same reason, avoid `==` between computed results and compare within a tolerance instead:
...
Float-to-integer conversions are always explicit and truncate toward zero; the reverse (int→float) widens implicitly. Casting `NaN` or `Inf` to an integer raises a [fatal error](/docs/lang/error/fatal) in debug builds.
...
- [`float`](/docs/lang/floating/float) — the alias used for `float64` in everyday code
- [`float32`](/docs/lang/floating/float32) — narrower, hardware-native alternative
