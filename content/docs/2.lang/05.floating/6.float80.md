...
Unsuffixed floating-point literals default to [`float64`](/docs/lang/floating/float64); request `float80` with the `f80` suffix or an annotation. Decimal and scientific forms (`1.5e10`) are both accepted — see [Literals](/docs/lang/lexical/literals).
...
Float-to-integer conversions are always explicit and truncate toward zero; casting `NaN` or `Inf` to an integer raises a [fatal error](/docs/lang/error/fatal) in debug builds.
...
- [`float64`](/docs/lang/floating/float64) — the default double-precision float
- [`float128`](/docs/lang/floating/float128) — the fallback type on non-x87 targets
