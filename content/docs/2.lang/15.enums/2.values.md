...
By default an enum's representation is chosen by the compiler. When the numeric value matters — for serialisation, file formats, or [interoperation with native code](/docs/lang/pointers/extern) — declare an integer **backing type** after the enum name and assign an explicit value to each variant:
...
Without an explicit backing type, do not rely on the numeric value of a variant — treat the enum as an opaque set of names and use [`match`](/docs/lang/statements/match) to branch on it.
...
- [Enumerations](/docs/lang/enums/overview) — the enum overview
- [Variants with Data](/docs/lang/enums/data) — carrying a payload instead of a fixed number
- [Pointers and `extern`](/docs/lang/pointers/extern) — exchanging values with native code
