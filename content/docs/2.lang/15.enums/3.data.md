...
The payload is **bound to names** by destructuring the variant in a [`match`](/docs/lang/statements/match). Because the match is exhaustive, every variant — and therefore every shape of data — must be handled:
...
The tag tells the compiler which interpretation of the payload is valid — which is exactly the safety an untagged [`union`](/docs/lang/unions/overview) does not provide.

## See Also

- [Enumerations](/docs/lang/enums/overview) — the enum overview
- [Backing Type and Explicit Values](/docs/lang/enums/values) — enums whose variants map to integers
- [`match`](/docs/lang/statements/match) — destructuring and the full pattern syntax
