...
A tuple groups a fixed number of values — possibly of different types — into a single value, without declaring a named [`struct`](/docs/lang/structs/overview). Tuples are written as a parenthesized, comma-separated list in both type position and value position.
...
A tuple is laid out like an unnamed [struct](/docs/lang/structs/overview#memory-layout): its elements are stored **in order**, each at the next offset that satisfies its [natural alignment](/docs/lang/appendix/primitives), with **padding** inserted where needed. The tuple's alignment is that of its strictest element, and its size is rounded up to a multiple of that alignment.
...
| Topic                                             | Description                                      |
| ------------------------------------------------- | ------------------------------------------------ |
| [Destructuring](/docs/lang/tuples/destructuring)  | Returning several values and unpacking them      |
| [Tuples vs. Structs](/docs/lang/tuples/vs-struct) | Choosing between positional and named aggregates |

## See Also

- [Structures](/docs/lang/structs/overview) — named aggregates with typed fields
- [Slices](/docs/lang/slices/overview) — sequences of a single element type
