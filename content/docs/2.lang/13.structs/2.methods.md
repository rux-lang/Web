...
The `extend` block attaches methods to a struct. **Instance methods** take `self` as their first parameter; **static methods** omit it. `self` is a [pointer](/docs/lang/pointers/members) to the structure, so a method reads and writes fields through it with `.`.
...
- [Construction and Field Access](/docs/lang/structs/overview#construction-and-field-access) — creating and reading values
- [Pointers to Fields and Members](/docs/lang/pointers/members) — how `self` is passed
