...
If a literal value does not fit in `int64`, the compiler emits an error at compile time. See [Literals](/docs/lang/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
The bitwise operators `&`, `|`, `^`, and `~` are also defined (`~x` equals `−(x + 1)`), but bitmask work usually reads more clearly on [unsigned types](/docs/lang/unsigned/uint64).
...
- [`uint64`](/docs/lang/unsigned/uint64) — unsigned counterpart of the same width
