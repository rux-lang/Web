...
A suffixed literal and an explicit annotation must agree; providing both when they conflict is a compile-time error. See [Literals](/docs/lang/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
Code that assumes a specific width for `int` will behave differently on 32-bit and 64-bit targets. When the bit width must be exact — serialised data, hardware registers, cross-platform binary protocols — use a fixed-width type such as [`int32`](/docs/lang/signed/int32) or [`int64`](/docs/lang/signed/int64) instead.
...
The bitwise operators `&`, `|`, `^`, and `~` are also defined (`~x` equals `−(x + 1)`), but bitmask work usually reads more clearly on [unsigned types](/docs/lang/unsigned/uint).

## Conversion

Rux performs no implicit numeric conversions — every conversion uses the [`as`](/docs/lang/operations/type-cast) operator. Because `int` has a target-dependent width, always cast to a fixed-width type before serialising or comparing across platforms.
...
- [`uint`](/docs/lang/unsigned/uint) — unsigned counterpart of the same width
