...
If a literal value does not fit in `int8`, the compiler emits an error at compile time. See [Literals](/docs/lang/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
The bitwise operators `&`, `|`, `^`, and `~` are also defined (`~x` equals `−(x + 1)`), but bitmask work usually reads more clearly on [unsigned types](/docs/lang/unsigned/uint8).

## Conversion

Rux performs no implicit numeric conversions — every conversion uses the [`as`](/docs/lang/operations/type-cast) operator. Widening to a wider signed type preserves the value through sign extension; a same-width signed↔unsigned cast reinterprets the bit pattern.
...
- [`uint8`](/docs/lang/unsigned/uint8) — unsigned counterpart of the same width
