...
A slice's [header](/docs/lang/slices/overview#memory-layout) exposes its `data` pointer and `length` directly, which is what makes low-level and foreign-code access possible without copying. An [array](/docs/lang/arrays/overview) exposes the same two fields, so everything here applies to both.

## Reaching the Backing Memory

A slice's `data` field is a regular `*T` pointer. You can pass it to [`extern`](/docs/lang/pointers/extern) functions or perform [pointer arithmetic](/docs/lang/pointers/arithmetic) on it when low-level access is needed:
...
- [Pointer Arithmetic](/docs/lang/pointers/arithmetic) — advancing a pointer by element size
