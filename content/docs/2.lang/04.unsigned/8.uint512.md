...
If a literal value does not fit in `uint512`, the compiler emits an error at compile time. See [Literals](/docs/lang/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
Rux performs no implicit numeric conversions — every conversion uses the [`as`](/docs/lang/operations/type-cast) operator. Narrowing keeps only the low-order bits; a same-width unsigned↔signed cast reinterprets the bit pattern. `uint512` is the widest unsigned type, so there is no wider type to widen into.
...
- [`int512`](/docs/lang/signed/int512) — signed counterpart of the same width
