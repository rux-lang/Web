...
If a literal value does not fit in `uint256`, the compiler emits an error at compile time. See [Literals](/docs/lang/lexical/literals) for decimal, hexadecimal, octal, and binary forms.
...
Rux performs no implicit numeric conversions — every conversion uses the [`as`](/docs/lang/operations/type-cast) operator. Widening zero-extends, preserving the value; narrowing keeps only the low-order bits; a same-width unsigned↔signed cast reinterprets the bit pattern.
...
- [`int256`](/docs/lang/signed/int256) — signed counterpart of the same width
