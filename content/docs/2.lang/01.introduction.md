...
`Main` is the [entry point](/docs/lang/functions/main) of every executable, and `import` brings `Print` from the standard library's `Std::Io` module into scope.

## Where to Go Next

- **New to Rux?** Follow the [Get Started guide](/docs/start) or try it in the browser on the [Playground](/play) — nothing to install.
- **Looking something up?** This reference is organized bottom-up: [lexical structure](/docs/lang/lexical/sources), [primitive types](/docs/lang/signed/int), [variables](/docs/lang/variables/overview) and [statements](/docs/lang/statements/overview), then [functions](/docs/lang/functions/overview), [user-defined types](/docs/lang/structs/overview), and [modules](/docs/lang/modules/overview).
- **Shipping a package?** Manifests, dependencies, and publishing live in [Packaging](/docs/packaging).
- **Calling native code?** See the [Foreign Function Interface](/docs/lang/ffi/overview) and the [attributes](/docs/lang/attributes/overview) that control linking and targets.
