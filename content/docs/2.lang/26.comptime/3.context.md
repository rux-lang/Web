...
The build context is a set of five [intrinsics](/docs/lang/comptime/intrinsic) the compiler fills in for each build. Every one is declared by the standard `Rux` package, so you **import** the ones you use:
...
Read a field with `.`; the field names are **lowerCamelCase**, and the query methods are **PascalCase**. Every value is fixed at compile time, so it can drive a [`when`](/docs/lang/comptime/conditional) condition or stand in as an ordinary literal in normal code.
...
The most important use of `#source` is as a **default argument**. A default expands at the _call site_, not where the function is defined, so a function can learn where it was called from without the caller passing anything. This is how the standard library's [`Assert`](/docs/api/core/assert) and [`Panic`](/docs/api/core/panic) capture a location:
...
Key/value strings supplied by the build — from a `[Build.Defines]` table in [`Rux.toml`](/docs/packaging/manifest) and overridden by `--define NAME[=VALUE]` on the command line.
...
The enum-valued fields draw from these variants. Only buildable operating systems can actually be produced; naming any other in a `when` is allowed but [warns](/docs/lang/comptime/conditional#naming-an-unbuildable-target).
...
| `OutputKind`                 | `Executable`, `SharedLibrary`, `StaticLibrary`, `SourceLibrary`                                                                                                                            |
...
- [Conditional Compilation](/docs/lang/comptime/conditional) — using these values in a `when`
- [Intrinsics](/docs/lang/comptime/intrinsic) — how `#target` and friends are declared and imported
- [Package Manifest](/docs/packaging/manifest) — the `[Build.Defines]` table behind `#config`
