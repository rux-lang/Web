...
`@` infers the pointer's mutability from the place it addresses: `@` on a `var` place yields a writable [`*var T`](/docs/lang/pointers/types), while `@` on a `let` place yields a read-only `*T`. Writing through the pointer therefore works only when the addressed value is itself mutable.
...
| Topic                                                | Description                                |
| ---------------------------------------------------- | ------------------------------------------ |
| [Pointer Types](/docs/lang/pointers/types)           | `*T`, `*var T`, nesting, and `*opaque`     |
| [The `null` Pointer](/docs/lang/pointers/null)       | The zero address and checking for no value |
| [Fields and Members](/docs/lang/pointers/members)    | Accessing struct fields through a pointer  |
