...
Writing through a pointer requires a writable pointer type, [`*var Point`](/docs/lang/pointers/types) — which `@p` produces only because `p` is a `var`. A plain `*Point` is read-only, and `ptr.x = 5.0` through it would be rejected.
...
See [Methods](/docs/lang/structs/methods) for the full method syntax.
...
A writable pointer parameter like `*var Vector` documents that the function mutates the caller's value, and the compiler only lets you pass `@dir` when `dir` is a `var`. When a function should read a struct but never modify it, use a plain read-only pointer, [`*Vector`](/docs/lang/pointers/types) — the default — so accidental writes are rejected.

## Self-Referential Structures

A struct cannot contain itself by value, but it can hold a **pointer** to its own type — the basis of linked lists, trees, and graphs. Terminate a chain with [`null`](/docs/lang/pointers/null), and always check for it before dereferencing:
...
- [Pointers](/docs/lang/pointers/overview) — the full pointer chapter
- [Address-of and Dereference](/docs/lang/pointers/overview#address-of-and-dereference) — taking addresses with `@`, reading with `*`
- [The `null` Pointer](/docs/lang/pointers/null) — representing and checking for no value
- [Methods](/docs/lang/structs/methods) — how `self` is passed
