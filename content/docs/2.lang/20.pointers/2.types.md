...
The **pointee type** follows the `*`. It determines how many bytes a dereference reads and writes, and how [pointer arithmetic](/docs/lang/pointers/arithmetic) steps through memory. The pointer itself is always one address wide; only the pointee type changes.
...
`*opaque` is a pointer to an unspecified type — the equivalent of `void*` in C. It carries an address with no element type, so it cannot be dereferenced directly and supports no [arithmetic](/docs/lang/pointers/arithmetic); cast it to a concrete pointer type first. It is most often used at [foreign function](/docs/lang/ffi/overview) boundaries:
...
- [Pointer Arithmetic](/docs/lang/pointers/arithmetic) — stepping by the pointee's size
- [Pointers and `extern`](/docs/lang/pointers/extern) — where `*opaque` and `*var T` map onto C
- [Type Casts](/docs/lang/operations/type-cast) — the `as` operator used to retype a pointer
