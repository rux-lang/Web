...
`char16` is a two-byte character covering the entire Basic Multilingual Plane (U+0000–U+FFFF) — Latin, Cyrillic, CJK, Arabic, Hebrew, and most common symbols. Like all Rux character types it is an unsigned, non-nullable value type that holds a single code point or raw code unit, and is type-distinct from both integers and strings. Use it when interoperating with UTF-16 APIs; otherwise prefer the default [`char`](/docs/lang/character/char).
...
- [`char8`](/docs/lang/character/char8) — the next width down
- [`char32`](/docs/lang/character/char32) — the next width up
