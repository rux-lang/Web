...
`char32` is the standard Unicode character type, covering all 1,114,112 code points (U+0000–U+10FFFF) including emoji and the supplementary planes. The plain [`char`](/docs/lang/character/char) keyword is an alias for it. Like all Rux character types it is an unsigned, non-nullable value type that holds a single code point or raw code unit, and is type-distinct from both integers and strings. It is the recommended type for general-purpose Unicode text.
...
- [`char`](/docs/lang/character/char) — the alias keyword for this type
- [`char16`](/docs/lang/character/char16) — the next width down
- [`char64`](/docs/lang/character/char64) — the next width up
