...
Shifts are most often used to build a mask that a [bitwise](/docs/lang/operations/bitwise) operator then applies — `1 << 3`, for instance, produces a value with only bit 3 set. See [Common Patterns](/docs/lang/operations/bitwise#common-patterns) for the standard set, clear, toggle, and test idioms that pair shifts with `&`, `|`, and `^`.
...
- [Bitwise Operations](/docs/lang/operations/bitwise) — `&`, `|`, `^`, `~` applied to shifted masks
- [Arithmetic Operations](/docs/lang/operations/arithmetic) — numeric operators on the same integer types
- [Unsigned Integer Types](/docs/lang/unsigned/uint8#shift-and-bitwise) — per-type shift behaviour
