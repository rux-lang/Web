...
See [Boolean Types](/docs/lang/boolean/bool) for the full conversion rules.

## Casting After a Type Test

`as` is the companion of the [`is`](/docs/lang/operations/type-test) type test. Test the type first, then cast inside the branch where the test has already succeeded:
...
`as` and `is` are companion [type operators](/docs/lang/lexical/operators#type-operators):
...
Parentheses are still worth adding when they make the intent clearer. See [Operator Precedence](/docs/lang/lexical/operators#operator-precedence) for the complete table.

## See Also

- [Type Tests](/docs/lang/operations/type-test) — the companion `is` operator
- [Arithmetic Operations](/docs/lang/operations/arithmetic) — where mismatched numeric types need a cast
- [Primitive Data Types](/docs/lang/appendix/primitives) — the types you convert between
