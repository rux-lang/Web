...
A **range** describes an interval between two bounds. It is written with the `..` and `..=` operators and used mainly to drive a [`for`](/docs/lang/statements/for) loop or to take a [sub-slice](/docs/lang/arrays/slicing#sub-slicing-with-a-range) of an array. A range is an ordinary value — it can be stored in a variable and its bounds read back.
...
| Use                                                              | Example         |
| ---------------------------------------------------------------- | --------------- |
| [Iterating](/docs/lang/ranges/usage#iteration) a counted loop    | `for i in 0..n` |
| [Sub-slicing](/docs/lang/ranges/usage#slicing) an array or slice | `data[1..4]`    |

Both are covered in [Using Ranges](/docs/lang/ranges/usage).

## See Also

- [Using Ranges](/docs/lang/ranges/usage) — ranges in `for` loops and slicing
- [`for` / `in`](/docs/lang/statements/for) — the loop a range most often drives
- [Arrays as Slices](/docs/lang/arrays/slicing) — taking a sub-slice with a range
