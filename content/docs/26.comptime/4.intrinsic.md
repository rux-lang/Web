...
The [`#source`](/docs/comptime/context#source) intrinsic exposes the current location — `#source.file`, `#source.line`, `#source.function`, and friends. Used as a [default argument](/docs/functions/declaration#default-arguments) it expands at the call site, letting a function capture where it was called from. This is the mechanism behind [`Assert`](/docs/api/rux/assert) and [`Panic`](/docs/api/rux/panic), and it replaces the earlier standalone `#file` / `#line` constants.
