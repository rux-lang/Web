...
The most important use of `#source` is as a **default argument**. A default expands at the _call site_, not where the function is defined, so a function can learn where it was called from without the caller passing anything. This is how the standard library's [`Assert`](/api/rux/assert) and [`Panic`](/api/rux/panic) capture a location:
