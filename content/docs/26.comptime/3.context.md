...
Key/value strings supplied by the build — from a `[Build.Defines]` table in [`Rux.toml`](/packaging/manifest) and overridden by `--define NAME[=VALUE]` on the command line.
...
- [Package Manifest](/packaging/manifest) — the `[Build.Defines]` table behind `#config`
