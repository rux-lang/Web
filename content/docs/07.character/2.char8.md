...
`char8` is a single-byte character for ASCII and the Latin-1 supplement (U+0000–U+00FF). Like all Rux character types it is an unsigned, non-nullable value type that holds a single code point or raw code unit, and is type-distinct from both integers and strings. Use it only for strict ASCII or Latin-1 data where the 1-byte constraint is a useful invariant; otherwise prefer the default [`char`](/docs/character/char).
...
- [`char16`](/docs/character/char16) — the next width up
