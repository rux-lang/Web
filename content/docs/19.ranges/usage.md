...
Indexing an [array](/docs/arrays/slicing) or [slice](/docs/slices/overview) with a range — rather than a single position — produces a **[slice](/docs/slices/overview)** over that sub-interval. Here _every_ range form is allowed: an omitted start defaults to `0`, and an omitted end defaults to the sequence's `length`.
