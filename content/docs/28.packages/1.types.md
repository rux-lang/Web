...
Every package has a type that tells the compiler what to produce. The type is set with the `Type` field in the [package manifest](/docs/packages/manifest).
...
Program and library packages have slightly different [directory layouts](/docs/packages/dirs).
