...
For a simple value like an `int`, the choice between [`let`](/docs/variables/let) and [`var`](/docs/variables/var) only decides whether the variable can be reassigned. For a [struct](/docs/structs/overview) it decides more: because a struct is a [value type](/docs/structs/overview#value-semantics), the binding that holds it governs **every field**. Mutability is _deep_ — there is no way to have an immutable struct with mutable fields, or the reverse.
