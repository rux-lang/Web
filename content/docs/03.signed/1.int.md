...
Code that assumes a specific width for `int` will behave differently on 32-bit and 64-bit targets. When the bit width must be exact — serialised data, hardware registers, cross-platform binary protocols — use a fixed-width type such as [`int32`](/docs/signed/int32) or [`int64`](/docs/signed/int64) instead.
