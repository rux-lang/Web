...
This is the key difference from a [slice](/docs/slices/overview): a slice is a 16-byte `{data, length}` _view_ into memory owned elsewhere, whereas an array _is_ the storage.
...
- [Tuples](/docs/tuples/overview) — fixed-size sequences of _mixed_ types
