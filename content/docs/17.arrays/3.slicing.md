...
An array owns its storage; a [slice](/docs/slices/overview) (`Slice<T>`) is a lightweight **view** into storage owned elsewhere. Because an array _is_ contiguous elements with a known length, it can always be viewed as a slice — and this is how you hand an array to code without copying it.
