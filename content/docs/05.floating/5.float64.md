...
`float64` is the default floating-point type in Rux — [`float`](/docs/floating/float) is a built-in alias for it. It is fully [IEEE 754](https://en.wikipedia.org/wiki/IEEE_754) compliant. Special values (`Inf`, `-Inf`, `NaN`) are supported, and division by zero produces `Inf` or `NaN` rather than a fatal error.
...
- [`float`](/docs/floating/float) — the alias used for `float64` in everyday code
- [`float32`](/docs/floating/float32) — narrower, hardware-native alternative
