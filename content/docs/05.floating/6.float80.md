...
Unsuffixed floating-point literals default to [`float64`](/docs/floating/float64); request `float80` with the `f80` suffix or an annotation. Decimal and scientific forms (`1.5e10`) are both accepted — see [Literals](/docs/lexical/literals).
...
- [`float64`](/docs/floating/float64) — the default double-precision float
- [`float128`](/docs/floating/float128) — the fallback type on non-x87 targets
