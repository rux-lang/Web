...
A dedicated macOS package is not available at this time. While native support is being worked on, you can run Rux on macOS by [building the compiler from source](/docs/start/build) — the build toolchain works anywhere a supported C++ compiler and CMake are available, including macOS.
