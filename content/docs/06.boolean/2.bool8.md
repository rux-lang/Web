...
`bool8` is the everyday boolean type, and the plain [`bool`](/docs/boolean/bool) keyword is an alias for it. Like every Rux boolean type it stores only `true` or `false`, regardless of its underlying width; the extra bits are zeroed and reserved. Booleans are distinct from integers — no implicit conversion happens in either direction, and a condition in `if`, `while`, or `for` must already be a boolean expression.
...
- [`bool`](/docs/boolean/bool) — the alias keyword for this type
- [`bool16`](/docs/boolean/bool16) — the next width up
