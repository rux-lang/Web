...
Code that assumes a specific width for `uint` will behave differently on 32-bit and 64-bit targets. When the bit width must be exact — serialised data, hardware registers, cross-platform binary protocols — use a fixed-width type such as [`uint32`](/docs/unsigned/uint32) or [`uint64`](/docs/unsigned/uint64) instead.
...
- [`uint32`](/docs/unsigned/uint32) / [`uint64`](/docs/unsigned/uint64) — the fixed-width types `uint` maps to
