...
- use `Type = "SourceLibrary"`; Rux 0.4.0 does not publish native binary artifacts.
...
::warning
Both `rux pack` and `rux publish` accept only SourceLibrary packages in Rux 0.4.0. They reject Executable, SharedLibrary, and StaticLibrary before credential lookup, archive creation, filesystem output, or network access. Binary distribution and binary dependency consumption will be enabled only by a later coordinated compiler and registry release.
::
