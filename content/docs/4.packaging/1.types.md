...
Every package has a type that tells the compiler what to produce. The type is set with the `Type` field in the [package manifest](/docs/packaging/manifest).
...
Program and library packages have slightly different [directory layouts](/docs/packaging/layout).
