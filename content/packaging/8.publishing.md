...
Publishing uploads a package release to the [registry](/packaging/registry), where anyone can depend on
...
- declare a [`Namespace`](/packaging/namespaces) you own or maintain;
- declare `MinRux`, the oldest compiler release that can build the package;
- describe a package, not a [workspace](/packaging/manifest#workspaces); and
...
You also need an [API token](/packaging/tokens) with the `publish` scope. Store it once with
[`rux login`](/cli/login):
...
Nothing about publishing builds or checks your package, so run [`rux check`](/cli/check) first.
...
[Versioning](/packaging/versioning) for how to choose the next number, and
[Yanking](/packaging/yanking) if you need to withdraw one.
...
| `no credential for …`                      | Run [`rux login`](/cli/login), or set `RUX_TOKEN`               |
| `namespace '…' is not claimed`             | [Claim it](/packaging/namespaces#claiming-a-namespace) first    |
| `does not own or maintain namespace '…'`   | Ask an owner to invite you                                      |
| `lacks the 'publish' scope`                | Create a token with the right [scope](/packaging/tokens#scopes) |
