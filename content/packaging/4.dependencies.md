...
[`[Dependencies]`](/packaging/manifest#dependencies) table of `Rux.toml` and managed with the `rux`
command-line tool.
...
with a registry dependency before you [publish](/packaging/publishing).
::
...
Remove one with [`rux remove`](/cli/remove):
...
[Versioning](/packaging/versioning) for how versions are ordered.
...
[`rux install`](/cli/install) and [`rux update`](/cli/update) and should not be edited by hand; use
[`rux uninstall`](/cli/uninstall) to remove entries.

## Workspaces

Inside a [workspace](/packaging/manifest#workspaces), a registry dependency that matches a member
